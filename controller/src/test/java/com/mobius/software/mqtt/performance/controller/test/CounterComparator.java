package com.mobius.software.mqtt.performance.controller.test;
...
public class CounterComparator
{
	...
	public static boolean compareByType(List<CommandCounter> counters, CommandType type)
	{
		Map<CommandType, Integer> outgoingMap = new HashMap<>();
		Map<CommandType, Integer> incomingMap = new HashMap<>();
		for (CommandCounter counter : counters)
		{
			if (counter.messageIsIncoming())
	...
	public static boolean comparePublish(List<CommandCounter> counters, Integer qos)
	{
		if (qos == 1 || qos == 2)
		{
			Map<CommandType, Integer> outgoingMap = new HashMap<>();
